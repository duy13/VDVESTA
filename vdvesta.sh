...
goc=`curl -L https://raw.githubusercontent.com/duy13/vDDoS-Protection/master/md5sum.txt --silent | grep "vddos-1.13.7-centos7" |awk 'NR==1 {print $1}'`
tai=`md5sum /usr/bin/vddos | awk 'NR==1 {print $1}'`
if [ "$goc" != "$tai" ]; then
curl -L http://1.voduy.com/vDDoS-Proxy-Protection/vddos-1.13.7-centos7 -o /usr/bin/vddos
fi
